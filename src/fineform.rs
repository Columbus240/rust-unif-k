...
impl FineForm {
    ...
    fn to_nnf_helper(p: (bool, usize)) -> NNF {
        let (b, i) = p;
        if b {
            NNF::AtomPos(i)
        } else {
    ...
    pub fn to_nnf(&self) -> NNF {
        if let FineForm::Node(node) = self {
            let mut atoms = node.atoms.iter().map(|x| FineForm::to_nnf_helper(*x));
    ...
    fn or(&self, other: &FineForm) -> FineForm {
        match (self, other) {
            ...
            (FineForm::Node(node0), FineForm::Node(node1)) => {
                let mut new_atoms = Vec::with_capacity(node0.atoms.len() + node1.atoms.len());
                new_atoms.extend_from_slice(&node0.atoms);
                new_atoms.extend_from_slice(&node1.atoms);
                FineForm::Node(Arc::new(FFNode {
                    atoms: new_atoms,
                    dia_branch: node0.dia_branch.as_ref().map_or(
                        node1.dia_branch.clone(),
                        |d0: &FineForm| {
                            node1
                                .dia_branch
                                .as_ref()
                                .map_or(Some(d0.clone()), |d1| Some(d0.or(&d1)))
                        },
                    ),
                    box_branches: node0
                        .box_branches
...
fn enumerate_step_nnf(input: Vec<Arc<NNF>>) -> Vec<Arc<NNF>> {
    ...
    for base in base_vectors {
        for set in powerset.iter() {
            ...
            for p in &base {
                disjuncts.insert(Arc::new(FineForm::to_nnf_helper(*p)));
            }
...
fn enumerate_step(input: Vec<FineForm>) -> Vec<FineForm> {
    ...
    for base in base_vectors {
        for set in powerset.iter() {
            ...
            for (b, f) in set {
                if *b {
                    dia_branch = dia_branch.or(&f);
                } else {
                    box_branches.insert(f.clone());
                }
            }
            let new_ff = FineForm::Node(Arc::new(FFNode {
                atoms: base.clone(),
                dia_branch: Some(dia_branch),
                box_branches: box_branches,
            }));

            // only add `new_ff` if no such element exists in
            // `output`
            if let Some(old_ff) = output
                .iter()
